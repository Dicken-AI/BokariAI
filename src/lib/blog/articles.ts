/**
 * Bokari blog — article model + (sample) content source.
 *
 * For now articles are statically defined here so the blog/article pages render
 * and are SEO-indexable. When the autonomous blog (10h cadence + WhatsApp
 * approval) ships, this module becomes the seam to a real store (Supabase),
 * keeping the same Article shape so the pages don't change.
 *
...
/** Up to `limit` other articles in the same category (excluding `slug`). */
export function getRelatedArticles(slug: string, categorySlug: string, limit = 3): Article[] {
